...
# ==========================
# 判断是否同一艘船（使用本地Qwen3.6模型，直接requests请求）
# ==========================
def is_same_ship(frame1, frame2):
    text = """
    你是一个专业的船舶识别专家。

    请根据船的细节特征判断两张图片中的船是否为同一艘。
    ...
    # 1. 将两张图片转换为 base64 编码的 data URL
    def frame_to_data_url(frame):
        _, buffer = cv2.imencode('.jpg', frame)
        base64_str = base64.b64encode(buffer).decode('utf-8')
        return f"data:image/jpeg;base64,{base64_str}"

    # 2. 构建符合 OpenAI API 格式的请求体
    payload = {
        ...
        "messages": [
            {
                ...
                "content": [
                    ...
                    },
                    {
                        "type": "image_url",
                        "image_url": {
                            "url": frame_to_data_url(frame1)
                        }
                    },
                    {
                        "type": "image_url",
                        "image_url": {
                            "url": frame_to_data_url(frame2)
                        }
                    }
                ]
            }
    ...
    # 3. 流式请求 llama-server（stream=True，SSE 解析）
    try:
        response = requests.post(
            'http://172.100.100.223:8080/v1/chat/completions',
    ...
    text_result = ''.join(content_parts).strip()

    print("\n模型原始输出：")
    print(text_result)

    # 去 markdown
    if text_result.startswith("```json") and text_result.endswith("```"):
