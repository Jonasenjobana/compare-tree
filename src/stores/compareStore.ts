...
import { updateRootName } from '@/api/tree'
...
export const useCompareStore = defineStore('compare', () => {
  ...
  const isSingleNodePreview = computed(() => treeStore.viewMode === 'tree' && !currentFrame.value)

  const compareTreeRootId = computed(() => {
    const tree = treeStore.allTrees[compareTreeIndex.value]
    return tree?.rootId || ''
  })
  ...
  async function updateTreeName(newName: string) {
    ...
    try {
      ...
      const tree = treeStore.allTrees.find((t) => t.rootId === rootId)
      if (tree) {
  ...
  async function loadCompareTreeByIndex(index: number) {
    const rootInfo = treeStore.allTrees[index]
    if (!rootInfo) return
    try {
      const res = await treeStore.loadTreeByRootId(rootInfo.rootId)
  ...
  function nextCompareGroup() {
    if (compareTreeIndex.value < treeStore.allTrees.length - 1) {
      loadCompareTreeByIndex(compareTreeIndex.value + 1)
    }
  }
  ...
  const hasMoreCompareTrees = computed(() => compareTreeIndex.value < treeStore.allTrees.length - 1)
  ...
  async function nextCompareTree() {
    if (compareTreeIndex.value < treeStore.allTrees.length - 1) {
      ...
      const rootInfo = treeStore.allTrees[nextIdx]
      if (!rootInfo) return
      try {
        const res = await treeStore.loadTreeByRootId(rootInfo.rootId)
        const tree = res!
  ...
  async function prevCompareTree() {
    if (compareTreeIndex.value > 0) {
      ...
      const rootInfo = treeStore.allTrees[prevIdx]
      if (!rootInfo) return
      try {
        const res = await treeStore.loadTreeByRootId(rootInfo.rootId)
        const tree = res!
  ...
  function handleCompareKeydown(e: KeyboardEvent) {
    if (treeStore.viewMode === 'timeline') {
  ...
  async function openCompareForNode(node: Node, childId?: string) {
    const idx = treeStore.allTrees.findIndex((t) => t.rootId == node.rootId)
    if (idx === -1) return

    try {
      const rootInfo = treeStore.allTrees[idx]!
      const res = await treeStore.loadTreeByRootId(rootInfo.rootId)
      const tree = res!
  ...
  watch(() => treeStore.selectedTreeId, async (rootId) => {
    if (!compareVisible.value || !rootId) return
    const idx = treeStore.allTrees.findIndex((t) => t.rootId === rootId)
    if (idx === -1 || idx === compareTreeIndex.value) return
    if (treeStore.viewMode === 'timeline') {
      loadCompareTreeByIndex(idx)
    } else {
      try {
        const res = await treeStore.loadTreeByRootId(rootId)
        const tree = res!
        if (!tree.tree || tree.tree.children.length === 0) {
          compareTreeIndex.value = idx
          compareTreeFrames.value = []
